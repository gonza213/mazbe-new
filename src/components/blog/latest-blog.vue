<template>
    ...
    <div v-if="blog" class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-[30px]">
        <div v-for="item in datas" :key="item"
            ...
            <div class="relative p-6">
                <div class="absolute start-6 -top-4">
                    <span class="bg-violet-600 text-white text-[12px] px-2.5 py-1 font-semibold rounded-full h-5">{{
        item.category }}</span>
                </div>

                <div class="">
                    <div class="flex mb-4">
                        <span class="text-slate-400 text-[16px]"><i
                                class="uil uil-calendar-alt text-slate-900 dark:text-white me-2"></i>{{ item.date
                            }}</span>
                        <span class="text-slate-400 text-[16px] ms-3"><i
                                class="uil uil-clock text-slate-900 dark:text-white me-2"></i>{{ item.time }}</span>
                    </div>

                    <router-link :to="{ name: 'blog-detail', params: { id: item.id } }"
                        class="title text-lg font-medium hover:text-violet-600 duration-500 ease-in-out">{{ item.title
                        }}</router-link>
                    ...
                    <div class="flex justify-between mt-4">
                        ...
                        <span class="text-slate-400 text-[16px]">by <router-link to="/creator-profile"
                                class="text-slate-900 dark:text-white hover:text-violet-600 dark:hover:text-violet-600 font-medium">{{
        item.by }}</router-link></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div v-else class="container lg:mt-24 mt-16">
        <div class="grid grid-cols-1 pb-8 text-center">
            <h6 v-if="minds" class="text-violet-600 font-medium mb-2">Our Minds</h6>
        ...
        <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 mt-8 gap-[30px]">
            <div v-for="item in datas.slice(0, 3)" :key="item"
                class="group relative overflow-hidden bg-white dark:bg-slate-900 rounded-md shadow dark:shadow-gray-700 hover:shadow-md transition-all duration-500">
                <img :src="item.image" alt="">
        ...
        item.category }}</span>
                    ...
                    <div class="">
                        ...
                        <router-link :to="{ name: 'blog-detail', params: { id: item.id } }"
                            class="title text-lg font-medium hover:text-violet-600 duration-500 ease-in-out">{{ item.title }}</router-link>
